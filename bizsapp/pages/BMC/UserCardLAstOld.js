...
export default function UserCard({ properties, userData, products }) {
    return (
        <>
            {properties && properties.map(properties => {

                console.log("card data", properties)
...
export async function getServerSideProps(context) {
    ...
    const productData = await db.collection("products").find({ user: ObjectId(id) }).toArray()
    // console.log("dataaaaaaaaaaaaa", data)

    const data2 = await db.collection("users").find({ _id: ObjectId(id) }).toArray()

    const themeData = await db.collection("theme").find({_id: data.theme}).toArray()
    console.log("themeDataaaaaaaaaaaaaa", themeData)
